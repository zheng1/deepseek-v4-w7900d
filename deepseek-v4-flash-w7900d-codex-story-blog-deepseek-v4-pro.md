# 一觉醒来，Codex 帮我把 DeepSeek-V4-Flash 跑在了 8 张 W7900D 上
...
llama-server 的多 slot 模式本来是提升并发的方向。一开始试 `-np 2` / `-np 4`，服务会在图构建阶段崩，堆栈落在 `llm_build_deepseek4 -> ggml_reshape_3d`。
...
这组是后来补上的。前面那些单请求或者小请求数的测试，能看单点延迟，但不够像“把机器压满”。所以又用 vLLM benchmark client 发了两条：一条单并发对照，一条并发 32、总共 100 个请求。
...
修完 multi-slot 崩溃后，我用同一个 `vllm bench serve` 口径补了一轮 `1024 input / 1 output` 的 prefill 压测。为了避免随机 prompt 场景下 prompt cache 添乱，这轮服务端关掉了 prompt cache 和 idle slot cache：
