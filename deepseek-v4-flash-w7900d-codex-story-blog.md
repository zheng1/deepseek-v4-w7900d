# 一觉醒来，Codex 把 DeepSeek-V4-Flash 跑在了 8 张 W7900D 上
...
所以现在的结论要改一下：`-np > 1` 不是不能碰，而是 DeepSeek4 multi-slot 必须走 unified KV。崩溃已经在本地 bati.cpp 里兜住了，后面要做的是补一轮标准 benchmark，而不是继续卡在启动阶段。
...
![Prefill 并发压测：1024 input / 1 output](assets/chart-prefill-c32.svg "Prefill 压测：并发 32 能把总吞吐压出来，但修复前 -np 1 会把 TTFT 拉高")
...
这组是后来补上的。前面那些单请求或者小请求数的测试，能看单点延迟，但不够像“把机器压满”。所以我又用 vLLM benchmark client 发了两条：一条单并发对照，一条并发 32、总共 100 个请求。

| 场景 | 请求数 | 并发 | Total tok/s | Mean TTFT | P95 TTFT |
|---|---:|---:|---:|---:|---:|
| 1024/1 | 10 | 1 | 72.47 | 14.14s | 14.34s |
| 1024/1 | 100 | 32 | 76.44 | 361.47s | 429.20s |

这组数字挺有意思：并发 32 确实把 prefill 总吞吐压到了 76.44 tok/s，比单并发 72.47 tok/s 高一点；但它没有让延迟变好，反而把 P95 TTFT 拉到了 429 秒。原因不神秘，这组是在修 multi-slot 之前用 `-np 1` 跑的，多并发请求虽然能一起进来，但主要还是排队执行。也就是说，这个结果适合拿来讲“单 slot 持续 prefill 吞吐基线”，不适合包装成“高并发低延迟服务已经毕业”。

自然下一问就是：那能不能把 `-np` 开大？

按 llama-server 的常规用法，`-np`/`--parallel` 确实就是 server slot 数。并发请求排队时，第一反应也应该是试 `-np 2`、`-np 4`，同时注意总 `--ctx-size` 会被多个 slot 分摊。如果每个请求都要 128K，那么 `-np 2` 理论上就该把 `-c` 提到 262K。

一开始实测确实不通。我试了两组：

| 配置 | 结果 |
|---|---|
| `-np 2 -c 131072 -b 2048 -ub 256` | 服务没到 `/health`，初始化阶段崩 |
| `-np 2 -c 32768 -b 512 -ub 256` | 同样初始化阶段崩 |

继续修以后，根因定位到非 unified KV cache 的多 stream 维度。现在本地补丁会在 DeepSeek4 multi-slot 下自动开启 unified KV cache。补丁后复测结果：

| 配置 | 结果 |
|---|---|
| `-np 2 -c 32768 -b 512 -ub 256` | `/health` 通过，2 路并发 smoke test 通过 |
| `-np 4 -c 16384 -b 512 -ub 256` | `/health` 通过，4 个 slot 都实际处理请求 |

这块算是从“启动就崩”推进到了“可以跑”。修完以后，我又用同一个 `vllm bench serve` 口径补了一轮 `1024 input / 1 output` 的 prefill 压测。为了避免随机 prompt 场景下 prompt cache 添乱，这轮服务端关掉了 prompt cache 和 idle slot cache：

```text
server = -np 4 -c 16384 -b 512 -ub 256
cache = off
input = 1024 tokens
output = 1 token
```
...
这组数字就比较诚实：`-np 4` 的多 slot 崩溃确实修好了，`c1` 的 prefill 也从老的 72.47 tok/s 提到了 86.09 tok/s。但把并发从 1 加到 4，并没有把总吞吐顶上去，反而把 TTFT 拉高了。原因大概率不在 HTTP 层，而在 layer split + ROCm + DeepSeek4 图执行本身：多 slot 能让请求同时被服务端接住，但底层 8 卡流水和 KV 管理并不会因此线性变快。

顺手还试了一个看起来更猛的配置：`-b 2048 -ub 512 --ctx-checkpoints 0`。结果单并发 prefill 只有 46.85 tok/s，长尾也很难看。所以当前推荐仍然是 `-b 512 -ub 256`，先稳住，不要被“大 batch 一定更快”骗了。
